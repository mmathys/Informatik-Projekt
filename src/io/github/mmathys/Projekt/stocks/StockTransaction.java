...
public class StockTransaction implements ErrorHandler, ActionHandler {
	...
	private void checkAmount(Question q) {
		...
		if (action == StockAction.BUY) {
			if (p.canBuy(value)) {
				p.buy(target, value);
				System.out.print("\n" + TerminalUtil.info("Du hast erfolgreich " + target.getName()
						+ "-Aktien im Wert von " + TerminalUtil.moneyFormat(value) + " gekauft."));
				TerminalUtil.continueIfEnterKeyPressed();
			} else {
				q.throwError(
						"Du hast nicht genug Kapital, um so viel von diesen Aktien zu kaufen. Dein Kapital beträgt "
								+ TerminalUtil.moneyFormat(p.getCapital()));
			}
		} else if (action == StockAction.SELL) {
			if (value > targetInvestment.getInvestmentValue()) {
				System.out.println(
						"So viel von diesen Aktien hast du nicht. Ich nehme an, du willst einfach alle verkaufen.");
				value = targetInvestment.getInvestmentValue();
			}

			p.sell(targetInvestment, value);

			System.out.print("\n" + TerminalUtil.info("Du hast erfolgreich deine " + target.getName()
					+ "-Aktien im Wert von " + TerminalUtil.moneyFormat(value) + " verkauft."));
			TerminalUtil.continueIfEnterKeyPressed();
		}
	}
