...
public class ViewAccountAction extends Action {
	...
	private void printAccountInfo() {
		...
		if (p.getCapital() >= 20000) {
			System.out.println(TerminalUtil.highlight(AnsiPrefixGenerator.getError()) + format(
					" ! Dein Chef wundert sich, wieso du so viel Geld in der Tasche hast und es nicht investierst.",
					Color.RED));
		}

		if (p.getCapital() > 20000) {
			System.out.println(TerminalUtil.highlight(AnsiPrefixGenerator.getHappy())
					+ format(" ! Dein Chef ist zufrieden mit dir. Du machst Gewinn.", Color.GREEN));
		} else if (p.getCapital() < 100/* keep capital */) {
			System.out.println(TerminalUtil.highlight(AnsiPrefixGenerator.getSad()) + format(
					" ! Dein Rücken kratzt, deine letzte Nacht hast du auf der Strasse verbracht, du hast kein Geld.",
					Color.RED));
		} else if (p.getCapital() < 20000) {
			System.out.println(TerminalUtil.highlight(AnsiPrefixGenerator.getSad()) + format(
					" ! Dein Chef überlegt sich, dich zu feuern mit deinen Finanzen sieht es im Moment nicht sehr gut aus.",
					Color.RED));
		}
